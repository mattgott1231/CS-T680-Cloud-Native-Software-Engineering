...
func NewVoterList() (*VoterList, error) {
	voterList := &VoterList{
		voterMap: make(DbMap),
		healthInfo: healthData{Uptime: time.Now(), APIcalls: 10},
	}
...
// GetVoterPollByPollId accepts a voter id and poll id and returns the requested poll.
// Preconditions:   (1) The database file must exist and be a valid
//
...
//		(3) The database file will not be modified
func (v *VoterList) GetVoterPollByPollId(voterId, pollId uint) (VoterPoll , error) {
...
func (v *VoterList) GetHealthData() (healthData, error){

	// data := healthData{Uptime: time.Now(), APIcalls: 10}
